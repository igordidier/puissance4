...
//2012-12-02 : Suppression du using namespace std puisque son usage est déconseillé.

int gagne(int c, int l, int J, int tab[7][6])  // renvoie J si le joueur gagne, sinon 1.
{
	//2012-12-02 : Optimisation du nombre de variables utilisées.
	int j[4];
	int k[3];

	//2012-12-02 : Optimisation de l'initialisation des tableaux
	for (int i = 0; i < 4; i++)
	{
		j[i] = 1;
	}
	...
	//2012-12-02 : Modification de la valeur renvoyé si aucun joueur ne gagne.
...
{
	//2012-12-02 : Modification de l'affichage pour que la console soit plus propre et claire.
	system("cls");
...
//2012-12-02 : Création d'une méthode pour propreté du code. Appeler lorsque l'utilisateur désire réellement jouer.
void jeu(int mode)
{
	//2012-12-02 : Optimisation du nombre de variables utilisées.
	int nbJetonsByColonne[7];   // donne le nombre de jetons sur chaque colonne

	//2012-12-02 : Ajout pour contenir le nom du ou des joueurs.
	...
	//2012-12-02 : Ajout pour gérer si on est en mode 2 joueurs ou en mode contre l'ordinateur
	...
	//2012-12-02:Ajout pour obtenir le nom du ou des joueurs qui joueront la partie.
	if (mode == 1)
	{
		mode2joueurs = true;
	...
	//2012-12-02 : Détermine qui débute la partie au hasard au lieu de choisir.
	srand(time(NULL));
	J = rand() % 2;
	if (J == 0)
	...
	while (g == -1)         //  tant que personne n'a gagné...
	{
		...
		//2012-12-02 : Ajout(s) pour la gestion du mode 2 joueurs.
		...
		else            //  si c'est l'ordi qui joue
		{
			...
			//2012-12-02 : Suppression d'une structure if complètement inutile et redondante.
			if (c == -1)
			{
				for (int j = 0; j < 7; j++)
				{
		...
		//2012-12-02: Modification pour gestion plus logique des tours des joueurs.
		if (J == 1)
			J--;
		else
			J++;
	...
	//2012-12-02: Modification des messages affichés et des valeurs pour chacune des cases puisque que la numérotation des joueurs a été changée.
	switch (g)
	{
	...
	case 0:std::cout << std::endl << "Bravo ! Tu as gagne " << tabJoueurs[0] << std::endl;
		...
		//2012-12-02: Ajout pour la gestion du mode 2 joueurs
...
int main()
{
	...
	//2012-12-02 : Redimensionnement de la fenêtre console pour un meilleur affichage.
	...
	//2012-12-02: Ajout d'un menu et de sa gestion pour les différents modes et pour quitter automatiquement.
	...
	//2012-12-02: Appel la méthode pour démarrer le jeu si l'utilisateur n'a pas choisi l'option quitter dans le menu.
	if (c != 0)
	{
		jeu(c);
	}
